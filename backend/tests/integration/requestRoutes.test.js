...
describe("POST /api/requests — max 5 active requests rule", () => {
  test("HTTP 400 on 6th active request", async () => {
    const loginRes = await login("req@g.com");
    const cookie = loginRes.headers["set-cookie"];
    ...
    expect(sixth.body.message).toBe(
      "You already have 5 active requests. Please complete or delete some before creating a new one."
    );
  });

  test("After deleting a request, user may create another", async () => {
    const loginRes = await login("req@g.com");
    const cookie = loginRes.headers["set-cookie"];
    ...
    const del = await request(app)
      .delete(`/api/requests/${createdIds[0]}`)
      .set("Cookie", cookie);

    expect(del.status).toBe(200);

    // should now be allowed to create one more
    const res = await request(app)
      .post("/api/requests")
      .set("Cookie", cookie)
      .send(payload);
